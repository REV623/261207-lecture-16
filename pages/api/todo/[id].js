...
export default async function todoIdRoute(req, res) {
  if (req.method === "DELETE") {
    ...
    */
    ...
    const result = await client.db("lecture-16").collection("todolist").findOneAndDelete({id: id})
    client.close()

    if(result.value === null) return res.status(404).json({ ok: false, message: "Todo is not found" });
    ...
    */
    ...
    const client = new MongoClient(process.env.MONGO_CONN_STR)
    await client.connect()
    const result = await client.db("lecture-16").collection("todolist").findOneAndUpdate({id: id},{$set: {completed: req.body.completed}})
    client.close()
